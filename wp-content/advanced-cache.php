...
$possible_plugin_locations = array(
	...
	'D:\_SERVER\domains\amazon-new\wp-content\plugins\wp-optimize\cache',
);
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', 'D:\_SERVER\domains\amazon-new\wp-content\plugins\wp-optimize\cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-amazon-new.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-amazon-new.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-amazon-new.php');
}
